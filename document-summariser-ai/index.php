...
<head>
  ...
  <title>Document Summariser AI | PDF and Image Summariser</title>
  <meta name="description" content="Summarise PDF files, images, and pasted documents with a practical browser-based AI document summariser.">
  ...
  <script type="application/ld+json">{"@context":"https://schema.org","@type":"WebApplication","name":"Document Summariser AI","url":"https://ai-document-summariser.faheeminnovations.online/","applicationCategory":"BusinessApplication","operatingSystem":"Any","description":"Summarise PDF files, images, and pasted documents."}</script>
  <style>
    :root {
      --bg: #f5f7fb;
      --panel: #ffffff;
...
<body>
  <header class="topbar">
    <div class="wrap brand-row">
      <div class="brand">
        ...
        <span>Document Summariser AI</span>
      </div>
      <nav class="nav">
        <a href="/faheeminnovations.online/tools">Back to tools</a>
      </nav>
    </div>
  </header>
  ...
  <main class="wrap hero">
    <section class="hero-grid">
      <div>
        <div class="eyebrow">AI summary tool</div>
        <h1>Turn long documents into short, clear insights.</h1>
        <p class="lead">Paste research notes, articles, reports, or meeting summaries and get a concise version in seconds. This version works instantly in the browser using a lightweight keyword-based summariser.</p>

        <div class="cta-row">
          <button class="btn primary" id="summarise-btn" type="button">Summarise text</button>
          <button class="btn secondary" id="sample-btn" type="button">Load sample</button>
          <button class="btn secondary" id="clear-btn" type="button">Clear</button>
          <label class="btn secondary" for="file-input" style="cursor:pointer;">Upload PDF / Image</label>
      ...
      <div class="panel">
        ...
        <textarea id="input-text" placeholder="Paste a long document, notes, or article here...">The team is planning a new AI workflow to improve customer support and reduce response time. The initiative includes better routing, automatic summarisation of incoming emails, and a shared knowledge base for staff. Early tests show faster replies and fewer repetitive questions. Business leaders also want stronger quality checks for answers, especially for sensitive customer issues. The next phase will compare cost, speed, and accuracy across tools before wider rollout. This plan is expected to help the company respond faster while keeping service consistent and trustworthy.</textarea>
        <div class="tools-row">
          <button class="btn secondary" id="copy-btn" type="button">Copy summary</button>
          <button class="btn secondary" id="download-btn" type="button">Download .txt</button>
        </div>
      </div>
    </section>
    ...
    <section class="section">
      <div class="summary-box">
        ...
        <div id="summary-output" class="summary-output">Your generated summary will appear here.</div>
      </div>
    </section>
    ...
    <section class="section">
      <div class="features">
        <div class="feature">
          ...
          <h4>Fast review</h4>
          <p>Get the main idea and key points without reading every sentence.</p>
        </div>
        <div class="feature">
          <div class="icon">2</div>
          <h4>Clean output</h4>
          <p>Summaries are simplified into the most important points and decisions.</p>
        </div>
        <div class="feature">
          <div class="icon">3</div>
          <h4>Share-ready</h4>
          <p>Copy or download the result for emails, notes, reports, or meetings.</p>
        </div>
      </div>
    </section>
  </main>
  ...
  <script>
    ...
    (function () {
      ...
      function renderSummary() {
        ...
        statusBadge.textContent = text.trim() ? 'Ready' : 'Empty';
      }
      ...
      async function extractTextFromPdf(file) {
        ...
        statusBadge.textContent = 'Scanning PDF image pages...';
        let ocrText = '';
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
          const page = await pdf.getPage(pageNum);
          ...
          const result = await Tesseract.recognize(canvas, 'eng+urd', {
            logger: (m) => {
              if (m && m.status && m.progress) {
                statusBadge.textContent = 'PDF OCR ' + Math.round(m.progress * 100) + '%';
              }
            }
          });
      ...
      async function extractTextFromImage(file) {
        ...
        statusBadge.textContent = 'Reading image...';
      ...
      async function handleUploadedFile(file) {
        ...
        try {
          ...
          if (isPdf) {
            ...
            statusBadge.textContent = 'Unsupported file';
            return;
          }

          if (!extracted || !extracted.trim()) {
            statusBadge.textContent = 'No readable text found';
            return;
          }

          input.value = extracted;
          renderSummary();
          statusBadge.textContent = 'File summarised';
        } catch (error) {
          console.error(error);
          statusBadge.textContent = 'Could not read file';
        }
      }

      summaryBtn.addEventListener('click', () => {
        renderSummary();
        statusBadge.textContent = 'Summary ready';
      });

      sampleBtn.addEventListener('click', () => {
        input.value = sampleText;
        renderSummary();
        statusBadge.textContent = 'Sample loaded';
      });
      ...
      copyBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          statusBadge.textContent = 'Copy blocked';
        }
      });
      ...
      downloadBtn.addEventListener('click', () => {
        ...
        if (!text || text === 'Your generated summary will appear here.') {
          statusBadge.textContent = 'No summary to download';
          return;
        }
